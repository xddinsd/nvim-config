local folderPath    = os.getenv("HOME") .. "/.config/nvim/lua/config/hotkey-scripts/open-terminal-in-tree"
...
function save_dir_under_nvim_tree_cursor()
    ...
    if variable_file then
        -- NOTE Change for zsh/fish
        -- variable_file:write(string.format('DIR_PATH="%s"\n', dir_path))
        -- variable_file:write(string.format('FILE_PATH="%s"\n', file_path))
        variable_file:write(string.format('set DIR_PATH "%s"\n', dir_path))
        variable_file:write(string.format('set FILE_PATH "%s"\n', file_path))
        variable_file:close()
    else
...
function open_folder_in_new_terminal()
    ...
    -- NOTE Change for your term
    -- os.execute("alacritty -o 'terminal.shell.program=\"" .. folderPath .. "/on_term_open.sh"     .. "\"'" .. " &") end
    os.execute("foot \"" .. folderPath .. "/on_term_open.fish"     .. "\"'" .. " &")
end
...
function open_file_in_nvim_in_new_terminal()
    ...
    -- NOTE Change for your term
    -- os.execute("alacritty -o 'terminal.shell.program=\"" .. folderPath .. "/on_term_open.sh" .. "\"'" .. " &") end
    print("foot \"" .. folderPath .. "/on_term_open.fish"     .. "\"'" .. " &")
    os.execute("foot \"" .. folderPath .. "/on_term_open.fish"     .. "\"" .. " &")
end
